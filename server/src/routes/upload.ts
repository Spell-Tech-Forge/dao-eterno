...
const storage = multer.diskStorage({
  destination: (req, _file, cb) => {
    ...
    const folder = t === 'monster' ? 'monsters' : t === 'biome' ? 'biomes' : t === 'frame' ? 'frames' : t === 'character' ? 'characters' : t === 'map' ? 'maps' : 'items'
...
router.post('/', upload.single('file'), (req, res) => {
  ...
  const t    = req.query.type as string
  const type = t === 'monster' ? 'monsters' : t === 'biome' ? 'biomes' : t === 'frame' ? 'frames' : t === 'character' ? 'characters' : t === 'map' ? 'maps' : 'items'
