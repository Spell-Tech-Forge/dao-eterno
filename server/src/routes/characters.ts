...
router.post('/:id/breakthrough', async (req, res) => {
  ...
  try {
    ...
    return res.json({
      ...result.rows[0],
      luck_gained: luckGain,
      talent_points_gained: talentPtsPerBT,
      attr_from_class: attrPointsPerBT,
      attr_from_agi_cap: agiCapBonus,
    })
  } catch (err) {
    await client.query('ROLLBACK')
    console.error(err)
...
router.post('/:id/die', async (req, res) => {
  ...
  try {
    ...
    const character_snapshot = {
      ...
      talent_points:     char.talent_points ?? 0,
      unlocked_talents:  char.unlocked_talents ?? {},
      unlocked_recipes:  char.unlocked_recipes ?? [],
      laws:              char.laws ?? {},
